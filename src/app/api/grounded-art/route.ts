...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── LATEST — for Shortcut wallpaper (instant) ─────────────────────
    if (action === 'latest') {
    ...
    // ── GENERATE — create new piece with overlay ──────────────────────
    if (action === 'generate') {
      ...
      if (!image_url || !title) {
        return NextResponse.json({ error: 'Missing image_url or title' }, { status: 500 })
      }
    ...
    // ── Pass other actions to edge function ───────────────────────────
...
export async function GET() {
  return NextResponse.json({
    ...
    shortcut: 'POST with X-API-KEY, body {action:"latest"}, ?format=image → JPEG wallpaper',
  })
}
