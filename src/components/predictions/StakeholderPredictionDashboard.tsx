...
export default function StakeholderPredictionDashboard({ organizationId }: { organizationId: string }) {
  ...
  const buildProfiles = async () => {
    try {
      ...
      // Simply reload predictions from database
      // The predictions are already being generated by the real-time monitor
      // No need to call stakeholder-profiler for bulk operations
      await loadPredictions()

      console.log('✅ Refreshed predictions from database')
    } catch (err: any) {
