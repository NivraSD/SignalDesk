...
export default function CrisisModule({ onOpenInStudio }: CrisisModuleProps) {
  ...
  const generateInitialTasks = (plan: CrisisPlan | null, scenario?: CrisisScenario) => {
    ...
    // Populate team from crisis plan
    if (plan?.crisisTeam) {
    ...
    // Generate tasks from scenario timeline
    if (scenario) {
      const timeline = getScenarioTimeline(scenario)
      timeline.forEach((item, idx) => {
        // Auto-assign based on action content keywords
        let assignee = plan?.crisisTeam?.[0]?.role || 'Crisis Response Leader'
        const actionLower = `${item.action} ${item.detail}`.toLowerCase()
        if (actionLower.includes('legal') || actionLower.includes('regulatory')) {
          assignee = plan?.crisisTeam?.find(m => m.role?.toLowerCase().includes('legal'))?.role || assignee
        } else if (actionLower.includes('security') || actionLower.includes('breach') || actionLower.includes('cyber')) {
          assignee = plan?.crisisTeam?.find(m => m.role?.toLowerCase().includes('ciso') || m.role?.toLowerCase().includes('security'))?.role || assignee
        } else if (actionLower.includes('media') || actionLower.includes('pr') || actionLower.includes('statement') || actionLower.includes('communication')) {
          assignee = plan?.crisisTeam?.find(m => m.role?.toLowerCase().includes('commun') || m.role?.toLowerCase().includes('pr'))?.role || assignee
        } else if (actionLower.includes('financial') || actionLower.includes('investor')) {
          assignee = plan?.crisisTeam?.find(m => m.role?.toLowerCase().includes('cfo') || m.role?.toLowerCase().includes('financ'))?.role || assignee
        }
    ...
    // Generate tasks from team member responsibilities
    if (plan?.crisisTeam) {
