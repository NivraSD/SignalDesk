...
export default function CampaignIntelligence() {
  ...
  // Listen for NIV handoff messages
  useEffect(() => {
    const handleNivHandoff = (event) => {
      if (event.data?.type === 'niv-handoff' && event.data?.target === 'campaign') {
        console.log('📨 Campaign Intelligence received NIV handoff:', event.data);

        const { payload } = event.data;
        if (payload) {
          setNivContext(payload);
          setShowNivInsights(true);

          // Auto-populate campaign fields from NIV context
          if (payload.context?.framework) {
            const framework = payload.context.framework;

            // Set campaign brief from objective
            if (framework.strategy?.objective) {
              setCampaignBrief(framework.strategy.objective);
            }

            // Auto-select category and type based on execution type
            if (framework.handoff?.executionType) {
              const executionType = framework.handoff.executionType;

              // Map execution types to categories
              if (executionType.includes('launch')) {
                setSelectedCategory('Product Launch');
              } else if (executionType.includes('crisis')) {
                setSelectedCategory('Crisis Management');
              } else if (executionType.includes('competitive')) {
                setSelectedCategory('Competitive Strategy');
              } else if (executionType.includes('thought-leadership')) {
                setSelectedCategory('Brand & Reputation');
                setSelectedType('thought-leadership');
              }
            }
          }
        }
