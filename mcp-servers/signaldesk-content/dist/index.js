            ...
            try {
                switch (name) {
                    case "generate_press_release": {
                        const { announcement_type, company_info, key_details, target_audience = 'general_business', tone = 'professional', include_boilerplate = true } = args;
                        // Generate press release structure
                        const headline = key_details.headline || `${company_info.name} Announces ${announcement_type.replace('_', ' ').replace(/\b\w/g, (l) => l.toUpperCase())}`;
                        ...
                        if (key_details.facts_figures && key_details.facts_figures.length > 0) {
                            ...
                            key_details.facts_figures.forEach((fact) => {
                                pressRelease += `• ${fact}\n`;
                            });
                            pressRelease += `\n`;
                        }
                        ...
                        if (key_details.quotes && key_details.quotes.length > 0) {
                            key_details.quotes.forEach((quote) => {
                        ...
                        await this.db.query(`INSERT INTO content_library (title, content_type, content_text, target_audience, tone, status)
               VALUES ($1, $2, $3, $4, $5, $6)`, [headline, 'press_release', pressRelease, target_audience, tone, 'draft']);
                        return {
                            content: [{
                                    type: "text",
                    ...
                    case "create_pitch_variants": {
                        const { base_story, target_outlets = [], angles, journalist_preferences = {}, urgency = 'timely' } = args;
                        ...
                        for (const variant of variants) {
                            await this.db.query(`INSERT INTO pitch_variants (variant_angle, subject_line, pitch_content, target_outlet)
                 VALUES ($1, $2, $3, $4)`, [variant.angle, variant.subject_line, variant.pitch_content, variant.target_outlets]);
                        }
                    ...
                    case "optimize_subject_lines": {
                        const { current_subject, email_type, target_audience = 'journalists', optimization_goals = ['higher_open_rate'], a_b_test_count = 5 } = args;
                        ...
                        optimizedSubjects.forEach((variant) => {
                    ...
                    case "generate_talking_points": {
                        const { topic, executive_role, event_type, key_messages = [], difficult_questions = [], time_limit, company_context = {} } = args;
                        ...
                        talkingPoints.main_points = (roleBasedPoints[executive_role] || [
                        ...
                        if (key_messages.length > 0) {
                            talkingPoints.key_messages_integration = key_messages.map((message) => `Opportunity to reinforce: "${message}" when discussing [relevant context]`);
                        }
                        // Address difficult questions
                        if (difficult_questions.length > 0) {
                            talkingPoints.difficult_qa = difficult_questions.map((question) => ({
                                question,
                        ...
                        // Store in database
                        await this.db.query(`INSERT INTO talking_points (topic, executive_role, event_type, main_points, supporting_data, difficult_questions, time_limit)
               VALUES ($1, $2, $3, $4, $5, $6, $7)`, [topic, executive_role, event_type, talkingPoints.main_points, talkingPoints.supporting_data, JSON.stringify(talkingPoints.difficult_qa), time_limit]);
                        ...
                        if (time_limit)
                            output += `Time Limit: ${time_limit} minutes\n`;
                        output += `\n${'='.repeat(50)}\n\n`;
                        output += `OPENING:\n${talkingPoints.opening}\n\n`;
                        output += `MAIN TALKING POINTS:\n`;
                        talkingPoints.main_points.forEach((point, index) => {
                            output += `${index + 1}. ${point}\n`;
                        });
                        output += `\n`;
                        output += `SUPPORTING DATA TO REFERENCE:\n`;
                        talkingPoints.supporting_data.forEach((data) => {
                            output += `• ${data}\n`;
                        });
                        output += `\n`;
                        if (key_messages.length > 0) {
                            output += `KEY MESSAGES TO INTEGRATE:\n`;
                            talkingPoints.key_messages_integration.forEach((integration) => {
                                output += `• ${integration}\n`;
                            });
                            output += `\n`;
                        }
                        if (difficult_questions.length > 0) {
                            ...
                            talkingPoints.difficult_qa.forEach((qa, index) => {
                    ...
                    case "create_crisis_statements": {
                        const { crisis_type, severity_level, affected_stakeholders, known_facts = [], company_actions = [], timeline_urgency = 'within_24_hours', legal_considerations = true } = args;
                        let statement = '';
                        let internalNotes = '';
                        ...
                        statement += openings[crisis_type] || `We want to address the recent situation regarding ${crisis_type.replace('_', ' ')}.`;
                        statement += `\n\n`;
                        // Add known facts
                        if (known_facts.length > 0) {
                            statement += `What we know:\n`;
                            known_facts.forEach((fact) => {
                                statement += `• ${fact}\n`;
                            });
                            statement += `\n`;
                        }
                        ...
                        if (company_actions.length > 0) {
                            ...
                            company_actions.forEach((action) => {
                                statement += `• ${action}\n`;
                            });
                            statement += `\n`;
                        }
                        ...
                        affected_stakeholders.forEach((stakeholder) => {
                            if (stakeholderMessages[stakeholder]) {
                                statement += `${stakeholderMessages[stakeholder]}\n`;
                            }
                        });
                        statement += `\n`;
                        ...
                        statement += timelineMessages[timeline_urgency];
                        statement += `\n\n`;
                        // Contact information
                        ...
                        await this.db.query(`INSERT INTO crisis_templates (crisis_type, severity_level, template_content, stakeholder_groups, approval_required)
               VALUES ($1, $2, $3, $4, $5)`, [crisis_type, severity_level, statement, affected_stakeholders, legal_considerations]);
                    ...
                    case "localize_content": {
                        const { source_content, target_markets, content_type, cultural_considerations = [], local_regulations = false, maintain_brand_voice = true } = args;
                        const localizedVersions = target_markets.map((market) => {
                            ...
                            cultural_considerations.forEach((consideration) => {
                    ...
                    case "fact_check_content": {
                        const { content_to_check, fact_types, verification_level = 'basic', source_requirements = [], urgency = 'standard', risk_tolerance = 'moderate' } = args;
                        ...
                        if (fact_types.includes('statistics')) {
                            ...
                            if (numbers) {
                                numbers.forEach((num) => extractedFacts.push({
                                    type: 'statistic',
                                    claim: num,
                        ...
                        if (fact_types.includes('dates')) {
                            ...
                            if (dates) {
                                dates.forEach((date) => extractedFacts.push({
                                    type: 'date',
                                    claim: date,
                                    status: 'needs_verification',
                                    confidence: 0
                        ...
                        if (fact_types.includes('financial_data')) {
                            ...
                            if (amounts) {
                                amounts.forEach((amount) => extractedFacts.push({
                                    type: 'financial_data',
                                    claim: amount,
                                    status: 'needs_verification',
                                    confidence: 0
                        ...
                        const verificationResults = extractedFacts.map((fact) => {
                        ...
                        for (const result of verificationResults) {
                            await this.db.query(`INSERT INTO fact_check_results (fact_claim, verification_status, confidence_level, checked_at, notes)
                 VALUES ($1, $2, $3, CURRENT_TIMESTAMP, $4)`, [result.claim, result.status, result.confidence, result.notes]);
                        }
