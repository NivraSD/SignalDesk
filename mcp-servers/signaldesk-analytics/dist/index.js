        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    case "calculate_media_value": {
                        const { coverage_items, time_period = 'month', include_social_amplification = false } = args;
                        let totalAVE = 0;
                        let totalReach = 0;
                        let totalImpressions = 0;
                        const calculations = coverage_items.map((item) => {
                            ...
                            if (item.prominence === 'front_page')
                                multiplier = 2.0;
                            else if (item.prominence === 'section_front')
                                multiplier = 1.5;
                            else if (item.prominence === 'brief')
                                multiplier = 0.5;
                        ...
                        return {
                            content: [{
                                    ...
                                    text: `Media Value Analysis (${time_period}):\n\n` +
                                        ...
                                        `BREAKDOWN BY OUTLET:\n` +
                                        calculations.map((calc) => `${calc.outlet} (${calc.type})\n` +
                    ...
                    case "sentiment_analysis": {
                        const { coverage_text, coverage_url, outlet, analysis_depth = 'basic', key_topics = [] } = args;
                        ...
                        // Store the analysis
                        if (coverage_url) {
                            await this.db.query(`INSERT INTO sentiment_analysis (overall_sentiment, key_themes, confidence_score)
                 VALUES ($1, $2, $3)`, [mockSentiment.overall_score, mockSentiment.themes, mockSentiment.confidence]);
                        }
                        return {
                    ...
                    case "competitive_share_of_voice": {
                        const { company_name, competitors, time_period = 'month', media_types = ['online', 'print'], keywords = [] } = args;
                    ...
                    case "campaign_roi_analysis": {
                        const { campaign_id, campaign_budget, goals, start_date, end_date, baseline_metrics = {} } = args;
                        ...
                        await this.db.query(`INSERT INTO campaigns (id, name, budget, start_date, end_date, goals, final_metrics, roi_score)
               ...
               final_metrics = $7, roi_score = $8, status = 'completed'`, [
                    ...
                    case "generate_executive_dashboard": {
                        const { report_type, metrics_focus = ['media_value', 'sentiment', 'reach'], audience = 'all_executives', include_recommendations = true } = args;
            ...
            `);
                    ...
                    case "track_message_penetration": {
                        const { key_messages, time_period = 'month', media_outlets = [], message_variations = true } = args;
                        ...
                        const mockResults = key_messages.map((message) => {
                        ...
                        const avgPenetration = mockResults.reduce((sum, r) => sum + parseFloat(r.penetration_rate), 0) / mockResults.length;
                        analysis += `SUMMARY:\n`;
                        analysis += `• Average Message Penetration: ${avgPenetration.toFixed(1)}%\n`;
                        analysis += `• Best Performing Message: #${mockResults.indexOf(mockResults.reduce((max, r) => parseFloat(r.penetration_rate) > parseFloat(max.penetration_rate) ? r : max)) + 1}\n`;
                        analysis += `• Messages needing reinforcement: ${mockResults.filter((r) => parseFloat(r.penetration_rate) < 30).length}\n\n`;
                        analysis += `RECOMMENDATIONS:\n`;
                        ...
                        for (let i = 0; i < key_messages.length; i++) {
                            await this.db.query(`INSERT INTO message_tracking (key_message, match_type, match_confidence)
                 VALUES ($1, $2, $3)`, [key_messages[i], 'penetration_analysis', mockResults[i].confidence / 100]);
                        }
                        return {
                            content: [{ type: "text", text: analysis }]
                        };
                    ...
                    case "coverage_quality_scoring": {
                        const { coverage_items, quality_factors = ['outlet_authority', 'journalist_expertise', 'message_accuracy'], benchmark_against = 'industry_average' } = args;
                        const qualityScores = coverage_items.map((item) => {
                            ...
                            if (item.content_length > 500)
                                score += 10;
                            if (item.mentions_count > 3)
                                score += 8;
                            if (item.key_message_inclusion)
                                score += 15;
                            if (item.expert_quotes)
                                score += 12;
                            if (item.multimedia_elements)
                                score += 8;
                            // Author expertise
                        ...
                        const avgScore = qualityScores.reduce((sum, item) => sum + item.quality_score, 0) / qualityScores.length;
                        ...
                        qualityScores.forEach((item, index) => {
                            ...
                            if (item.author)
                                analysis += `   Author: ${item.author}\n`;
                            analysis += `   Factors: `;
                            const factors = [];
                            if (item.key_message_inclusion)
                                factors.push('Key Messages ✅');
                            if (item.expert_quotes)
                                factors.push('Expert Quotes ✅');
                            if (item.multimedia_elements)
                                factors.push('Multimedia ✅');
                            if (item.content_length > 500)
                                factors.push('Comprehensive ✅');
                            analysis += factors.length > 0 ? factors.join(', ') : 'Basic coverage';
                            analysis += '\n\n';
                        });
                        const topTier = qualityScores.filter((item) => item.quality_score >= 80).length;
                        const lowQuality = qualityScores.filter((item) => item.quality_score < 60).length;
