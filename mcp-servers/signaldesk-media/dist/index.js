    ...
    try {
        switch (name) {
            case 'find_journalists': {
                const { beat, publication, location, recentCoverage = true, limit = 10 } = args;
        ...
        Return ONLY valid JSON array.`;
            ...
            case 'analyze_journalist': {
                const { journalistName, publication } = args;
        ...
        Return as JSON object.`;
            ...
            case 'create_media_list': {
                const { listName, topic, tier1Publications = [], targetBeats = [] } = args;
        ...
        Return as JSON array with: name, publication, beat, email, whyRelevant`;
            ...
            case 'monitor_coverage': {
                const { keywords = [], competitors = [], timeframe = '7d' } = args;
            ...
            case 'generate_pitch': {
                const { journalistName, storyAngle, companyInfo = '', pitchType = 'immediate' } = args;
        ...
        Body: [email body]`;
            ...
            case 'track_outreach': {
                const { action, journalistId, status, notes } = args;
