  ...
  try {
    switch (name) {
      case 'find_journalists': {
        const { beat, publication, limit = 10 } = args as any;
      ...
      case 'monitor_coverage': {
        const { keywords = [], timeframe = '7d' } = args as any;
        
        const coverage = {
          timeframe,
          totalArticles: Math.floor(Math.random() * 50) + 10,
          keywordMentions: Object.fromEntries(
            keywords.map((k: string) => [k, Math.floor(Math.random() * 20) + 1])
          ),
          topPublications: [
