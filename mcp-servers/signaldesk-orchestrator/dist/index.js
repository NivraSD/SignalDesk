...
class OrchestratorServer {
            ...
            try {
                switch (name) {
                    case 'orchestrate_parallel_analysis':
                        return await this.orchestrateParallelAnalysis(args);
                    case 'route_to_specialist':
                        return await this.routeToSpecialist(args);
                    default:
    ...
    async orchestrateParallelAnalysis(args) {
        const { organization, findings, analysis_config = {} } = args;
        const { parallel = true, timeout_per_mcp = 20000, mcps_to_use = ['intelligence', 'monitor', 'media', 'analytics', 'scraper'], depth = 'standard' } = analysis_config;
        ...
        if (mcps_to_use.includes('intelligence')) {
            analysisPromises.push(this.callWithTimeout(mcpClients.intelligence.call('analyze_competition_with_personality', {
                findings: taggedFindings.filter((f) => f.relevance.competition > 0.5),
                organization,
                analysis_depth: depth
            }), timeout_per_mcp, 'intelligence'));
        }
        if (mcps_to_use.includes('monitor')) {
            analysisPromises.push(this.callWithTimeout(mcpClients.monitor.call('analyze_trending_with_sarah', {
                findings: taggedFindings.filter((f) => f.relevance.trending > 0.5),
                organization,
                analysis_depth: depth
            }), timeout_per_mcp, 'monitor'));
        }
        if (mcps_to_use.includes('media')) {
            analysisPromises.push(this.callWithTimeout(mcpClients.media.call('analyze_stakeholders_with_victoria', {
                findings: taggedFindings.filter((f) => f.relevance.stakeholder > 0.5),
                organization,
                analysis_depth: depth
            }), timeout_per_mcp, 'media'));
        }
        if (mcps_to_use.includes('analytics')) {
            analysisPromises.push(this.callWithTimeout(mcpClients.analytics.call('analyze_market', {
                findings: taggedFindings.filter((f) => f.relevance.market > 0.5),
                organization,
                analysis_depth: depth
            }), timeout_per_mcp, 'analytics'));
        }
        if (mcps_to_use.includes('scraper')) {
            analysisPromises.push(this.callWithTimeout(mcpClients.scraper.call('detect_cascades', {
                findings: taggedFindings.filter((f) => f.relevance.cascade > 0.7),
                organization,
                analysis_depth: 'deep' // Always deep for cascade detection
            }), timeout_per_mcp, 'scraper'));
        }
...
${JSON.stringify(combinedResults, null, 2)}`,
    ...
    async routeToSpecialist(args) {
        ...
        const client = mcpClients[targetMCP];
    ...
    calculateRelevance(finding, type) {
    ...
    async callWithTimeout(promise, timeout, mcpName) {
        return Promise.race([
            promise,
            new Promise((_, reject) => setTimeout(() => reject(new Error(`${mcpName} timeout after ${timeout}ms`)), timeout))
        ]);
    }
