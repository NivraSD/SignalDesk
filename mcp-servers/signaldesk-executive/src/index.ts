...
class ExecutiveSummaryServer {
      ...
      try {
        switch (name) {
          case 'synthesize_executive_summary':
            return await this.synthesizeExecutiveSummary(args as any);
          case 'generate_insight_dashboard':
            return await this.generateInsightDashboard(args as any);
          case 'generate_action_matrix':
            return await this.generateActionMatrix(args as any);
          default:
  ...
  private async synthesizeExecutiveSummary(args: any) {
...
Provide an executive synthesis in this JSON structure:
{
  ...
  private renderDashboard(synthesis: any, analyses: any, organization: any): string {
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div class="action-matrix">
      ...
      <div class="grid">
        <div>
          ...
          ${(synthesis.strategic_recommendations?.do_now || []).map((action: string) => 
            `<div class="recommendation do-now">${action}</div>`
          ).join('')}
        </div>
        
        <div>
          <strong>🔵 Prepare For (1 Week)</strong>
          ${(synthesis.strategic_recommendations?.prepare_for || []).map((action: string) => 
            `<div class="recommendation prepare">${action}</div>`
          ).join('')}
        </div>
        
        <div>
          <strong>🟣 Watch For (Ongoing)</strong>
          ${(synthesis.strategic_recommendations?.watch_for || []).map((signal: string) => 
            `<div class="recommendation watch">${signal}</div>`
          ).join('')}
        </div>
        
        <div>
          <strong>🔴 Avoid (Risks)</strong>
          ${(synthesis.strategic_recommendations?.avoid || []).map((risk: string) => 
            `<div class="recommendation avoid">${risk}</div>`
          ).join('')}
        </div>
      </div>
    </div>
  ...
  private renderCompetitionInsights(competition: any): string {
    if (!competition) return '<p>No competition data available</p>';
  ...
  private renderTrendingInsights(trending: any): string {
    if (!trending) return '<p>No trending data available</p>';
  ...
  private renderStakeholderInsights(stakeholders: any): string {
    if (!stakeholders) return '<p>No stakeholder data available</p>';
  ...
  private generateNarrative(synthesis: any, analyses: any): string {
  ...
  private renderHTML(synthesis: any, analyses: any, organization: any): string {
...
<p>${synthesis.narrative_summary}</p>`;
  ...
  private renderMarkdown(synthesis: any, analyses: any, organization: any): string {
...
${(synthesis.integrated_insights?.cross_domain_patterns || []).map((p: string) => `- ${p}`).join('\n')}

### Hidden Connections
${(synthesis.integrated_insights?.hidden_connections || []).map((c: string) => `- ${c}`).join('\n')}

### Cascade Implications
${(synthesis.integrated_insights?.cascade_implications || []).map((i: string) => `- ${i}`).join('\n')}
...
### 🟠 Do Now (24 Hours)
${(synthesis.strategic_recommendations?.do_now || []).map((a: string) => `- ${a}`).join('\n')}

### 🔵 Prepare For (1 Week)
${(synthesis.strategic_recommendations?.prepare_for || []).map((p: string) => `- ${p}`).join('\n')}

### 🟣 Watch For (Signals)
${(synthesis.strategic_recommendations?.watch_for || []).map((w: string) => `- ${w}`).join('\n')}

### 🔴 Avoid (Risks)
${(synthesis.strategic_recommendations?.avoid || []).map((a: string) => `- ${a}`).join('\n')}
...
*Synthesizing insights from: Marcus Chen, Sarah Rodriguez, Victoria Chen, Market Expert, Cascade Specialist*`;
  ...
  private renderReport(synthesis: any, analyses: any, organization: any): string {
    // Full report format - would be much longer in production
    return this.renderMarkdown(synthesis, analyses, organization);
  }

  private async generateInsightDashboard(args: any) {
  ...
  private async generateActionMatrix(args: any) {
  ...
  private extractAndPrioritizeActions(analyses: any, timeHorizon: string): any {
