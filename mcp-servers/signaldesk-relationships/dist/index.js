            ...
            try {
                switch (name) {
                    case "track_journalist_interactions": {
                        const { journalist_id, interaction_type, interaction_details, outcome = 'neutral', follow_up_needed = false } = args;
                        // Record the interaction
                        const result = await this.db.query(`INSERT INTO journalist_interactions (journalist_id, interaction_type, interaction_details, outcome, follow_up_needed)
               VALUES ($1, $2, $3, $4, $5)
               RETURNING *`, [journalist_id, interaction_type, interaction_details, outcome, follow_up_needed]);
               ...
               WHERE id = $1`, [journalist_id, scoreAdjustment]);
                    ...
                    case "relationship_health_scoring": {
                        const { journalist_id, timeframe = 'quarter', include_recommendations = true } = args;
            ...
            `;
                      ...
                      ORDER BY j.relationship_score DESC`;
                        ...
                        if (result.rows.length === 0) {
                            ...
                            output += retryResult.rows.map((row) => {
                    ...
                    case "optimal_outreach_timing": {
                        const { journalist_id, outlet_type, message_type } = args;
                        if (journalist_id) {
...
` : ''}`
                    ...
                    case "gift_and_ethics_tracking": {
                        const { journalist_id, gift_type, estimated_value, description, compliance_notes = '' } = args;
                        // Record the gift/entertainment
                        const result = await this.db.query(`INSERT INTO ethics_tracking (journalist_id, gift_type, estimated_value, description, compliance_notes)
               VALUES ($1, $2, $3, $4, $5)
               RETURNING *`, [journalist_id, gift_type, estimated_value, description, compliance_notes]);
            ...
            `, [journalist_id]);
                    ...
                    case "journalist_career_tracking": {
                        const { journalist_id, track_new_position, new_outlet, new_beat, start_date } = args;
                        if (track_new_position && journalist_id) {
                            // End current position
                            await this.db.query(`UPDATE journalist_positions 
                 SET is_current = FALSE, end_date = CURRENT_DATE
                 WHERE journalist_id = $1 AND is_current = TRUE`, [journalist_id]);
                            // Add new position
                            await this.db.query(`INSERT INTO journalist_positions (journalist_id, outlet, beat, start_date)
                 VALUES ($1, $2, $3, $4)`, [journalist_id, new_outlet, new_beat, start_date || new Date().toISOString().split('T')[0]]);
                            // Update journalist record
                            await this.db.query(`UPDATE journalists 
                 SET outlet = $2, beat = $3, updated_at = CURRENT_TIMESTAMP
                 WHERE id = $1`, [journalist_id, new_outlet, new_beat]);
                            return {
                                content: [{
                                        type: "text",
            ...
            `);
              ...
              `);
                    ...
                    case "relationship_warming_suggestions": {
                        const { relationship_status = 'all', priority_outlets = [], industry_focus } = args;
                        let query = `
            ...
            `;
                    ...
                    case "influencer_relationship_mapping": {
                        const { influencer_type = 'all', platform, follower_range = 'all', industry_relevance } = args;
                        let query = `
            ...
            `;
                        ...
                        if (follower_range !== 'all') {
                            ...
                            const range = ranges[follower_range];
                            if (range) {
                        ...
                        return {
                            content: [{
                                    type: "text",
                                    text: `Influencer Relationship Map:\n\n${result.rows.map((inf) => `${inf.name} (@${inf.handle})\nPlatform: ${inf.platform}\nFollowers: ${inf.follower_count?.toLocaleString() || 'N/A'}\nEngagement: ${inf.engagement_rate || 'N/A'}%\nRelevance: ${inf.industry_relevance}\nRelationship Score: ${inf.relationship_score}/100\nLast Contact: ${inf.last_interaction || 'None'}\n`).join('\n---\n')}`
                                }]
                        };
                    }
                    default:
    ...
    async createSampleJournalists() {
        ...
        for (const journalist of journalists) {
            await this.db.query(`INSERT INTO journalists (name, email, outlet, beat, twitter_handle, relationship_score, last_interaction)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`, [journalist.name, journalist.email, journalist.outlet, journalist.beat, journalist.twitter, journalist.score, '2025-01-10']);
        }
    }
    async createSampleCareerMoves() {
        ...
        for (const move of moves) {
            await this.db.query(`INSERT INTO journalist_positions (journalist_id, outlet, beat, start_date)
         VALUES ($1, $2, $3, $4)`, [move.journalist_id, move.outlet, move.beat, move.start_date]);
        }
    }
    async createSampleInfluencers() {
        ...
        for (const influencer of influencers) {
            await this.db.query(`INSERT INTO influencers (name, influencer_type, platform, handle, follower_count, engagement_rate, industry_relevance, relationship_score)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [influencer.name, influencer.type, influencer.platform, influencer.handle, influencer.followers, influencer.engagement, influencer.relevance, influencer.score]);
        }
    }
    async run() {
