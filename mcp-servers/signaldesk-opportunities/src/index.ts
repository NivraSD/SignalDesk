      ...
      try {
        switch (name) {
          case "discover_opportunities": {
            const { industry, keywords, limit = 10 } = args as any;
          ...
          case "analyze_opportunity": {
            const { opportunity_id } = args as any;
...
Recommendation: ${opp.score >= 80 ? 'HIGH PRIORITY - Act quickly' : opp.score >= 60 ? 'MEDIUM PRIORITY - Worth pursuing' : 'LOW PRIORITY - Monitor for now'}`
          ...
          case "create_opportunity": {
            const { title, type, description, urgency = 'medium', deadline, keywords = [] } = args as any;
            
            const result = await this.db!.query(
              `INSERT INTO opportunities (title, type, description, urgency, deadline, keywords, score)
               VALUES ($1, $2, $3, $4, $5, $6, $7)
               RETURNING *`,
              [title, type, description, urgency, deadline, keywords, 75] // Default score
            );
          ...
          case "track_opportunity": {
            const { opportunity_id, notes } = args as any;
            
            await this.db!.query(
              `INSERT INTO opportunity_tracking (opportunity_id, notes)
               VALUES ($1, $2)`,
          ...
          case "get_opportunity_trends": {
            const { timeframe = 'week' } = args as any;
            
            let interval = '7 days';
            if (timeframe === 'today') interval = '1 day';
              ...
              content: [
                {
                  ...
                  text: `PR Opportunity Trends (${timeframe}):\n\n${result.rows.map((row: any) => 
                    `${row.type}: ${row.count} opportunities (avg score: ${Math.round(row.avg_score)})`
                  ).join('\n')}`
                }
              ]
            };
          ...
          case "suggest_pitch": {
            const { opportunity_id, company_context } = args as any;
...
Timing: ${opp.urgency === 'high' ? 'Send immediately' : 'Send within 24-48 hours'}`
  ...
  private async createSampleOpportunities() {
    const samples = [
      {
    ...
    for (const opp of samples) {
      await this.db!.query(
        `INSERT INTO opportunities (title, type, description, score, urgency, deadline, keywords, relevant_journalists, suggested_action)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
        [opp.title, opp.type, opp.description, opp.score, opp.urgency, opp.deadline, 
