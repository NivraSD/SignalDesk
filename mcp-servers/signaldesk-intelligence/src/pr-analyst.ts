...
export class PRAnalyst {
  ...
  async analyzeSignals(signals: Signal[], organization: any): Promise<AnalysisResult> {
    ...
    const strategic_implications = await this.deriveStrategicImplications(
      signal_analysis,
      pattern_recognition,
      stakeholder_impact
    );
    ...
    // 5. Response Strategy
    const response_strategy = await this.developResponseStrategy(
      strategic_implications,
      stakeholder_impact
    );
    
    // 6. Elite Insights (the secret sauce)
    const elite_insights = await this.generateEliteInsights(
      signals,
      pattern_recognition,
      strategic_implications
    );
  ...
  private async analyzeIndividualSignals(signals: Signal[], organization: any): Promise<SignalAnalysis[]> {
  ...
  private deepAnalyzeSignal(signal: Signal, organization: any) {
  ...
  private scoreRelevance(signal: Signal, organization: any): number {
    let score = 0;
    
    // Direct mention of organization
    if (signal.title?.toLowerCase().includes(organization.name.toLowerCase())) score += 40;
    if (signal.content?.toLowerCase().includes(organization.name.toLowerCase())) score += 20;
    
    // Competitor mention
    if (signal.entity_type === 'competitor') score += 30;
    
    // Industry relevance
    if (signal.title?.toLowerCase().includes(organization.industry?.toLowerCase())) score += 20;
    
    // Keyword matches
    const keywords = organization.keywords || [];
    keywords.forEach((keyword: string) => {
      if (signal.title?.toLowerCase().includes(keyword.toLowerCase())) score += 10;
    });
    
    return Math.min(score, 100);
  }

  private assessMagnitude(signal: Signal, organization: any): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private assessVelocity(signal: Signal): 'slow' | 'moderate' | 'fast' | 'viral' {
    // Time-based velocity assessment
    if (!signal.published) return 'moderate';
    
    const hoursAgo = (Date.now() - new Date(signal.published).getTime()) / (1000 * 60 * 60);
    
    if (hoursAgo < 1) return 'viral';
    if (hoursAgo < 6) return 'fast';
    if (hoursAgo < 24) return 'moderate';
    
    return 'slow';
  }

  private extractFactual(signal: Signal): string {
    // Extract the core fact from the signal
    return signal.title || 'Unknown event occurred';
  }

  private extractMeaning(signal: Signal, organization: any): string {
  ...
  private recommendAction(signal: Signal, magnitude: string, velocity: string): string {
  ...
  private async identifyPatterns(signals: Signal[], analyses: SignalAnalysis[]): Promise<Pattern[]> {
  ...
  private identifyTrendingTopics(signals: Signal[]) {
    const topics = new Map<string, { count: number; signals: string[] }>();
  ...
  private identifyCascadePatterns(signals: Signal[], analyses: SignalAnalysis[]): Pattern[] {
    const patterns: Pattern[] = [];
    
    // Look for high-velocity, high-magnitude signals that could cascade
    const triggerSignals = analyses.filter(a => 
      a.magnitude === 'high' || a.magnitude === 'critical'
    );
  ...
  private assessCustomerImpact(analyses: SignalAnalysis[], patterns: Pattern[]): StakeholderImpact {
  ...
  private assessInvestorImpact(analyses: SignalAnalysis[], patterns: Pattern[]): StakeholderImpact {
  ...
  private assessMediaImpact(analyses: SignalAnalysis[], patterns: Pattern[]): StakeholderImpact {
  ...
  private assessEmployeeImpact(analyses: SignalAnalysis[], patterns: Pattern[]): StakeholderImpact {
  ...
  private assessPartnerImpact(analyses: SignalAnalysis[], patterns: Pattern[]): StakeholderImpact {
    return {
  ...
  private assessRegulatoryImpact(analyses: SignalAnalysis[], patterns: Pattern[]): StakeholderImpact {
    return {
  ...
  private async deriveStrategicImplications(
    analyses: SignalAnalysis[],
    patterns: Pattern[],
    stakeholderImpact: StakeholderMatrix
  ...
  private assessReputationState(analyses: SignalAnalysis[], stakeholder: StakeholderMatrix): string {
    const highConcern = Object.values(stakeholder).filter(s => s.concern_level === 'high').length;
    if (highConcern >= 3) return 'Under pressure from multiple stakeholders';
    if (highConcern >= 1) return 'Localized reputation challenges';
    return 'Generally positive reputation';
  }

  private assessReputationTrajectory(analyses: SignalAnalysis[], patterns: Pattern[]): 'improving' | 'stable' | 'declining' | 'crisis' {
    const criticalSignals = analyses.filter(a => a.magnitude === 'critical').length;
    if (criticalSignals > 0) return 'crisis';
    
    const highVelocity = analyses.filter(a => a.velocity === 'viral' || a.velocity === 'fast').length;
    if (highVelocity > 3) return 'declining';
    
    const cascadeRisks = patterns.filter(p => p.type === 'cascade_risk').length;
    if (cascadeRisks > 2) return 'declining';
    
    return 'stable';
  }

  private assessInterventionNeed(analyses: SignalAnalysis[], patterns: Pattern[]): 'none' | 'monitor' | 'respond' | 'urgent' {
    ...
    if (highCount > 2) return 'respond';
    if (highCount > 0) return 'monitor';
    
    return 'none';
  }

  private identifyVulnerabilities(analyses: SignalAnalysis[], patterns: Pattern[]): string[] {
  ...
  private assessCompetitiveStrength(analyses: SignalAnalysis[], patterns: Pattern[]): 'leader' | 'challenger' | 'follower' | 'at_risk' {
    ...
    if (ourMentions > competitorMentions * 2) return 'leader';
    if (ourMentions > competitorMentions) return 'challenger';
    if (competitorMentions > ourMentions * 2) return 'at_risk';
    
    return 'follower';
  }

  private assessCompetitiveMomentum(analyses: SignalAnalysis[], patterns: Pattern[]): 'gaining' | 'maintaining' | 'losing' {
    const recentHighImpact = analyses.filter(a => 
      a.magnitude === 'high' && a.velocity === 'fast'
    ).length;
    
    if (recentHighImpact > 3) return 'losing';
    if (recentHighImpact > 1) return 'maintaining';
    
    return 'gaining';
  }

  private identifyAdvantages(analyses: SignalAnalysis[]): string[] {
  ...
  private identifyExposedFlanks(analyses: SignalAnalysis[], patterns: Pattern[]): string[] {
    const flanks = [];
  ...
  private identifyControlledNarratives(analyses: SignalAnalysis[]): string[] {
    return ['Customer success stories', 'Company culture', 'Leadership vision'];
  }

  private identifyCompetitorNarratives(analyses: SignalAnalysis[]): string[] {
    return analyses
  ...
  private identifyContestedNarratives(analyses: SignalAnalysis[]): string[] {
    return ['Market leadership', 'Innovation pace', 'Customer satisfaction'];
  }

  private identifyNarrativeOpportunities(patterns: Pattern[]): string[] {
    return patterns
      .filter(p => p.type === 'narrative_shift')
      .map(p => `Lead conversation on: ${p.insight}`);
  }

  private async developResponseStrategy(
  ...
  private createActionPlan(
    ...
    const plans = {
      ...
      short: {
        priority: (priority === 'critical' ? 'high' : priority) as 'low' | 'medium' | 'high' | 'critical',
        actions: [
    ...
    return plans[timeframe as keyof typeof plans] || plans.immediate;
  }

  private async generateEliteInsights(
    signals: Signal[],
    patterns: Pattern[],
    implications: StrategicImplications
  ): Promise<EliteInsights> {
  ...
  private findHiddenConnections(signals: Signal[], patterns: Pattern[]): string[] {
    const connections = [];
  ...
  private findSimultaneousEvents(signals: Signal[]): Signal[] {
    // Group signals by time window
    const timeGroups = new Map<string, Signal[]>();
  ...
  private findInversePatterns(signals: Signal[]): string[] {
    ...
    const neglected = allTopics.filter(topic => 
      !competitorFocus.some(focus => focus.includes(topic))
    );
    
    return neglected;
  }

  private extractTopic(signal: Signal): string {
    const text = (signal.title + ' ' + signal.content).toLowerCase();
  ...
  private findAsymmetricOpportunities(signals: Signal[], patterns: Pattern[]): string[] {
    const opportunities = [];
  ...
  private identifyCommonThreats(signals: Signal[]): string[] {
  ...
  private identifyLeveragePoints(patterns: Pattern[], implications: StrategicImplications): string[] {
  ...
  private identifyBlindspots(
    ...
    const geoMentions = signals.filter(s => 
      s.title.includes('Asia') || s.title.includes('Europe') || s.title.includes('America')
    );
    ...
    const futureMentions = signals.filter(s => 
      s.title.includes('2025') || s.title.includes('2026') || s.title.includes('future')
    );
