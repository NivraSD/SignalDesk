...
export class CascadePredictor {
  ...
  // Test function to verify cascade predictions
  testPredictions() {
    const scenarios = [
    ...
    console.log('Testing Cascade Predictor...\n');
    
    scenarios.forEach(scenario => {
      const result = this.predictCascade(scenario.type, scenario.details);
      console.log(`${scenario.type.toUpperCase().replace('_', ' ')}:`);
      console.log(`- Confidence: ${(result.confidence * 100).toFixed(0)}%`);
      console.log(`- Opportunities: ${result.opportunities.length}`);
      console.log(`- Top Action: ${result.opportunities[0]?.action || 'None'}`);
      console.log('');
    });
  }
}

export default CascadePredictor;