...
-- Handles the INTEGER id issue and ensures proper column creation order
...
-- ========================================
-- UPDATE OPPORTUNITIES TABLE FIRST
-- ========================================
...
-- Drop existing opportunity_queue if it exists (to avoid partial state issues)
...
-- Create indexes for opportunity_queue AFTER table is created
CREATE INDEX idx_opportunity_queue_status ON opportunity_queue(status, priority_score DESC);
CREATE INDEX idx_opportunity_queue_org ON opportunity_queue(organization_id, queued_at DESC);
...
-- Update niv_conversations to add organization_id (only if table exists)
DO $$ 
BEGIN
    -- Check if niv_conversations table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_name = 'niv_conversations') THEN
        -- Add organization_id if missing
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Update niv_messages to add metadata (only if table exists)
DO $$ 
BEGIN
    -- Check if niv_messages table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables 
...
BEGIN
    RAISE NOTICE '===========================================';
    RAISE NOTICE 'MemoryVault Migration Complete (Safe Version)';
    RAISE NOTICE '===========================================';
    RAISE NOTICE '';
    RAISE NOTICE 'Key Changes:';
    RAISE NOTICE '- Fixed campaigns.opportunity_id to use INTEGER (matching opportunities.id)';
    RAISE NOTICE '- Reordered operations to update opportunities table BEFORE creating opportunity_queue';
    RAISE NOTICE '- Added DROP TABLE IF EXISTS for opportunity_queue to handle partial states';
    RAISE NOTICE '- Removed IF NOT EXISTS from opportunity_queue indexes';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '- niv tables (organization links - if tables exist)';
END $$;