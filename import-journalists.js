// Import journalists from Journalists.md into journalist_registry table
...
// Journalist data from Journalists.md
const JOURNALIST_REGISTRY = {
  technology: [
...
async function importJournalists() {
  console.log('📊 Importing journalists into database...\n');

  let totalImported = 0;
  let totalErrors = 0;

  for (const [industry, journalists] of Object.entries(JOURNALIST_REGISTRY)) {
    console.log(`\n📁 Importing ${industry} journalists (${journalists.length})...`);
      ...
      try {
        const { data, error } = await supabase
          ...
          .insert({
            ...
            tier: 'tier1', // All from this list are tier 1
            enrichment_status: 'pending'
          })
          .select();

        if (error) {
          console.error(`  ❌ Error importing ${journalist.name}:`, error.message);
          totalErrors++;
        } else {
          console.log(`  ✅ Imported: ${journalist.name} (${journalist.outlet})`);
          totalImported++;
        }
      } catch (err) {
        console.error(`  ❌ Exception importing ${journalist.name}:`, err.message);
        totalErrors++;
      }
    }
  }
