...
// ============================================================================
// PRESENTATION MODE - Intelligent presentation generation from conversation
// ============================================================================

/**
 * Structure for presentation outline generated by Claude
 */
...
/**
 * Detect if user wants to create a presentation based on conversation
 */
function detectPresentationIntent(
  userMessage: string,
  ...
  const directRequest = /\b(create|make|generate|build)\s+(a\s+)?(presentation|deck|slides?|ppt)\b/i.test(userMessage) ||
                       /\b(presentation|deck|slides?)\s+(about|for|on)\b/i.test(lower) ||
                       /\bmake\s+(this\s+)?(a\s+)?(deck|pres|presentation|slides?)\b/i.test(lower) ||
                       /\bturn\s+.*(into|to)\s+(a\s+)?(presentation|deck|slides?)\b/i.test(lower)

  if (directRequest) return true

  // Check if confirming a pending outline
  ...
  // Check if asking to modify an existing outline
  const isModification = /(add|remove|change|modify|update)\s+.*(slide|section|point)/i.test(lower) ||
                        /more\s+(detail|slides?|sections?)/i.test(lower) ||
...
/**
 * Extract presentation outline stored in conversation history
 * Looks in multiple places: response metadata, message outline field, or legacy HTML comment
 */
function extractOutlineFromHistory(history: any[]): PresentationOutline | null {
  // Search backward through history for the outline
  for (let i = history.length - 1; i >= 0; i--) {
    const msg = history[i]
    if (msg.role === 'assistant' || msg.role === 'niv') {
      // First check if outline is in message metadata (preferred - cleaner)
      if (msg.outline && typeof msg.outline === 'object' && msg.outline.topic) {
        console.log('✅ Found outline in message metadata')
        return msg.outline as PresentationOutline
      }

      // Check if there's a response type indicating presentation_outline
      if (msg.type === 'presentation_outline' && msg.outline) {
        console.log('✅ Found outline from presentation_outline response type')
        return msg.outline as PresentationOutline
      }

      // Legacy fallback: look for HTML comment marker in content
      const match = msg.content?.match(/<!-- PRESENTATION_OUTLINE\n([\s\S]*?)\n-->/)
      if (match) {
        try {
          console.log('✅ Found outline from legacy HTML comment')
          return JSON.parse(match[1])
        } catch (e) {
          console.warn('Failed to parse outline from HTML comment:', e)
        }
      }
    }
  }
...
/**
 * Check if user is confirming to generate the presentation
 */
function isUserConfirmingGeneration(userMessage: string): boolean {
  const confirmPatterns = [
    /^(yes|yeah|yep|sure|ok|okay)\.?$/i,
    /\b(go ahead|proceed|do it|generate|generate it|looks good|perfect|let'?s do it|approved?)\b/i
  ]
  return confirmPatterns.some(p => p.test(userMessage.trim()))
}

/**
 * Generate presentation outline using Claude to analyze conversation
 */
async function generatePresentationOutline(
  ...
  console.log('📊 Generating presentation outline via Claude analysis...')
  ...
  const prompt = `You are analyzing a conversation to create a presentation outline.
...
ORGANIZATION: ${organizationName}${industry ? ` - ${industry}` : ''}

Based on this conversation, extract the following for a presentation:

1. TOPIC: What is this presentation about? (concise title)
2. SUBTITLE: A compelling one-liner for the subtitle
3. AUDIENCE: Who will see this presentation?
4. PURPOSE: What should they think/feel/do after?
5. KEY_MESSAGES: 3-5 main points discussed in the conversation
6. SLIDE_COUNT: Recommend 8-12 slides based on content depth
7. SECTIONS: 8-12 slide sections, each with:
   - title: Section/slide title
   - talking_points: 2-4 bullet points (extracted from conversation)
   - visual_suggestion: What kind of visual would work (optional)

IMPORTANT:
- Extract real content from the conversation, don't make things up
- Structure the presentation logically with a clear narrative flow
- Include concrete details, data points, and examples from the conversation
- The sections should tell a coherent story
...
Return ONLY valid JSON matching this schema:
{
  ...
  const outline = JSON.parse(jsonMatch[0]) as PresentationOutline

  // Validate and set defaults
  outline.slide_count = outline.slide_count || outline.sections?.length || 10
  outline.key_messages = outline.key_messages || []
...
/**
 * Format outline as clean, structured markdown preview for user review
 * Note: We DON'T include hidden JSON here - the outline is returned as a separate response field
 */
function formatOutlinePreview(outline: PresentationOutline): string {
  let preview = `## 📊 Presentation Outline\n\n`

  // Header section with key info
  preview += `| | |\n|---|---|\n`
  preview += `| **Topic** | ${outline.topic} |\n`
  preview += `| **Subtitle** | ${outline.subtitle} |\n`
  preview += `| **Audience** | ${outline.audience} |\n`
  preview += `| **Purpose** | ${outline.purpose} |\n`
  preview += `| **Slides** | ${outline.slide_count} |\n\n`

  // Key messages section
  preview += `### Key Messages\n\n`
  outline.key_messages.forEach((msg, i) => {
    preview += `${i + 1}. ${msg}\n`
  })
  preview += `\n`

  // Slide structure - more compact
  preview += `### Slide Structure\n\n`
  outline.sections.forEach((section, i) => {
    preview += `**${i + 1}. ${section.title}**\n`
    section.talking_points.slice(0, 3).forEach(point => {
      preview += `- ${point}\n`
    })
    if (section.talking_points.length > 3) {
      preview += `- *(+${section.talking_points.length - 3} more)*\n`
    }
    preview += `\n`
  })

  preview += `---\n\n`
  preview += `✅ Say **"generate"** or **"looks good"** to create the presentation in Gamma.\n`
  preview += `✏️ Or tell me what you'd like to change.`

  return preview
}

/**
 * Convert outline to Gamma-optimized markdown content
 */
function outlineToGammaMarkdown(outline: PresentationOutline): string {
  let content = `# ${outline.topic}\n\n`
  content += `${outline.subtitle}\n\n`

  // Key messages as intro content
  if (outline.key_messages.length > 0) {
    content += outline.key_messages.slice(0, 2).join('\n\n') + '\n\n'
  }

  // Each section becomes a slide
  for (const section of outline.sections) {
    content += `# ${section.title}\n\n`
    for (const point of section.talking_points) {
      content += `- ${point}\n`
    }
 ...
 * Routes presentation requests through intelligent outline generation
 */
async function handlePresentationMode(
  ...
  // Check if user is confirming an existing outline
  const existingOutline = extractOutlineFromHistory(conversationHistory)
  const isConfirmation = isUserConfirmingGeneration(userMessage)

  if (existingOutline && isConfirmation) {
    // User approved the outline - generate presentation in Gamma
    console.log('✅ User confirmed outline - generating Gamma presentation...')
    return generateGammaPresentationFromOutline(
      existingOutline,
      organizationId,
      sessionId,
      conversationId
  ...
  // Generate new outline (or regenerate if user requested changes)
  try {
    ...
    // Return outline preview for user review
    const preview = formatOutlinePreview(outline)
    ...
    return new Response(
      JSON.stringify({
        success: false,
        type: 'error',
        message: `Failed to generate presentation outline: ${error.message}`,
        sessionId,
        conversationId
      }),
      {
...
/**
 * Generate Gamma presentation from approved outline
 */
async function generateGammaPresentationFromOutline(
  ...
  try {
    ...
    const gammaContent = outlineToGammaMarkdown(outline)

    // Create folder path for Memory Vault
    const cleanTitle = (outline.topic || 'Untitled').replace(/[^\w\s-]/g, '').substring(0, 50)
    ...
    // NOTE: Memory Vault save is handled by gamma-presentation edge function
    // when frontend polls with capture=true. This avoids duplicate records.
    console.log(`📁 Gamma will save to Memory Vault when complete (org: ${organizationId}, folder: ${folderPath})`)

    const responseMessage = `🎨 **Creating your presentation...**

Your ${outline.slide_count}-slide presentation "${outline.topic}" is being generated in Gamma.

This typically takes 30-60 seconds. I'll update you when it's ready!`

    return new Response(
      JSON.stringify({
        success: true,
        type: 'gamma_generating',
...
async function getMcpDiscovery(organizationInput: string = '7a2835cb-11ee-4512-acc3-b6caf8eb03ff') {
  console.log('🎯 Loading org profile from organizations table for:', organizationInput)
  ...
  try {
    ...
    if (orgData) {
      ...
      console.log(`✅ Found org profile: ${orgData.name}`)
      console.log(`  Industry: ${companyProfile.industry || orgData.industry || 'Unknown'}`)
      console.log(`  Description: ${(companyProfile.description || '').substring(0, 100)}...`)
      console.log(`  Competitors: ${(companyProfile.competition?.direct_competitors || []).slice(0, 3).join(', ')}`)

      // Return in the format the rest of the code expects
      return {
        organization_name: orgData.name,
        organization_id: orgData.id,
        industry: companyProfile.industry || orgData.industry || 'Unknown',
        description: companyProfile.description || '',
        competition: companyProfile.competition || { direct_competitors: [], indirect_competitors: [] },
        keywords: companyProfile.key_markets || [orgData.name],
    ...
    console.log(`⚠️ No org found for "${organizationInput}", using default profile`)
    return createDefaultProfile(organizationInput)
  } catch (error) {
...
function detectCommandIntent(message: string): { isCommand: boolean; commandType?: string } {
  ...
  const routingCommands = [
    ...
    /^(yes|yeah|yep|sure|ok|okay|go ahead|proceed|do it|generate it|approved?|let'?s do it|sounds? good|perfect)\.?$/i  // Confirmation responses
  ]
...
What would you like to explore or work on today?`
    ...
    // =====================================================================
    // PRESENTATION MODE - Intelligent presentation routing (NEW)
    // Check BEFORE other routing commands to capture presentation requests
    // IMPORTANT: Only run on 'full' stage, not 'acknowledge' stage
    // =====================================================================
    const isPresentationRequest = detectPresentationIntent(userMessage, conversationHistory)
    if (isPresentationRequest && stage !== 'acknowledge') {
      console.log('📊 Presentation Mode activated - routing to intelligent handler...')
      return handlePresentationMode(
        userMessage,
        conversationHistory,
        organizationId,
        organizationName,
    ...
    if (commandDetection.isCommand && commandDetection.commandType === 'routing') {
      ...
      // Use flexible pattern matching - don't require exact keywords
      for (let i = conversationHistory.length - 1; i >= 0; i--) {
        const msg = conversationHistory[i]
        if (msg.role === 'assistant' || msg.role === 'niv') {
          // Check if this message contains strategic content - FLEXIBLE matching
          const content = msg.content || ''
          const contentLower = content.toLowerCase()

          // More flexible detection - look for common strategic patterns
          const hasStrategy =
            // Exact phrase matches (original)
            content.includes('STRATEGIC OBJECTIVES') ||
            content.includes('Strategy:') ||
            content.includes('Tactical Plan') ||
            content.includes('Key Messages') ||
            content.includes('Timeline') ||
            // Case-insensitive pattern matches (new - more flexible)
            contentLower.includes('strategic ') ||
            contentLower.includes('recommend') ||
            contentLower.includes('approach:') ||
            contentLower.includes('position') ||
            contentLower.includes('objective') ||
            contentLower.includes('messaging') ||
            contentLower.includes('target audience') ||
            contentLower.includes('action items') ||
            contentLower.includes('next steps') ||
            // Markdown headers suggesting structure
            /^#{1,3}\s+.*(strategy|approach|recommendation|objective|plan|message)/im.test(content) ||
            // Numbered/bulleted lists with strategic content
            /\d+\.\s*(position|message|audience|tactic)/i.test(content) ||
            // Long substantive content (>500 chars with multiple paragraphs = likely strategic)
            (content.length > 500 && content.split('\n\n').length >= 2)

          if (hasStrategy) {
            console.log('📦 Found strategic advice in conversation (flexible match)')

            // Try to extract a title from the content
            const titleMatch = content.match(/^#{1,3}\s+(.+)/m) ||
                              content.match(/\*\*(.{10,60}?)\*\*/) ||
                              content.match(/^([A-Z][^.!?\n]{15,80})/m)
            const extractedTitle = titleMatch ? titleMatch[1].trim() : 'Strategic Initiative'

            // Extract key points from numbered/bulleted lists
            const listItems = content.match(/(?:^|\n)\s*(?:\d+\.|[-•*])\s+(.+)/g) || []
            const keyMessages = listItems
            ...
            latestFramework = {
              strategy: {
                objective: extractBetween(content, 'Primary Goal:', '\n') ||
                          extractBetween(content, 'Objective:', '\n') ||
                          extractedTitle,
                narrative: content,  // Full context
                keyMessages: extractListItems(content, 'Key Messages:').length > 0
                  ? extractListItems(content, 'Key Messages:')
                  : keyMessages,
              ...
              contentStrategy: {
                subject: extractBetween(content, 'Primary Goal:', '\n') ||
                        ...
                        extractedTitle,
                narrative: content,
                key_messages: extractListItems(content, 'Key Messages:').length > 0
                  ? extractListItems(content, 'Key Messages:')
                  : keyMessages,
                target_audiences: extractListItems(content, 'Target Audience'),
      ...
      // Detect what content type user wants - check BOTH current message AND conversation history
      ...
      // If user is confirming (short message like "yes"), check conversation history for context
      const isConfirmation = /^(yes|yeah|yep|sure|ok|okay|go ahead|proceed|do it|generate it|approved?|let'?s do it|sounds? good|perfect)\.?$/i.test(userMessage.trim())

      if (isConfirmation && !useGamma) {
        // Check if there's a prior presentation context in conversation
        const recentMessages = conversationHistory.slice(-6)
        const hasPresentationContext = recentMessages.some((msg: any) => {
          const content = (msg.content || '').toLowerCase()
          return content.includes('presentation') ||
                 content.includes('deck') ||
                 content.includes('slides') ||
                 content.includes('gamma') ||
                 content.includes('ready to generate') ||
                 content.includes('confirmation_needed')
        })

        if (hasPresentationContext) {
          console.log('📋 User confirming prior presentation request')
          requestedContentType = 'presentation'
          useGamma = true
        }
      }
      ...
      if (latestFramework) {
        ...
        try {
          ...
          if (useGamma && requestedContentType === 'presentation') {
            ...
            // More flexible check: user gave confirmation AND NIV previously offered to generate
            const userIsConfirming = /\b(yes|yeah|yep|sure|ok|okay|confirm|go ahead|proceed|do it|generate it|approved?|let'?s do it|sounds? good|perfect)\b/i.test(userMessage)

            // Explicit generation commands bypass the confirmation requirement entirely
            // "create the presentation", "generate the deck", "make the slides" etc.
            const isExplicitGenerationCommand = /\b(create|generate|make|build)\s+(the\s+)?(presentation|deck|slides?|ppt)\b/i.test(userMessage)

            const nivAskedForConfirmation = conversationHistory.some((msg: any) => {
              if (msg.role !== 'niv' && msg.role !== 'assistant') return false
              const content = (msg.content || '').toLowerCase()
              return content.includes('ready to generate') ||
                     content.includes('here\'s what i\'ll include') ||
                     content.includes('say **"yes"**') ||
                     content.includes('say "yes"') ||
                     content.includes('confirmation_needed') ||
                     content.includes('generate, or tell me')
            })

            // User is either: confirming after NIV asked, OR giving an explicit generation command
            const isExplicitConfirmation = (userIsConfirming && nivAskedForConfirmation) || isExplicitGenerationCommand

            console.log(`🎨 Gamma confirmation check: userIsConfirming=${userIsConfirming}, nivAskedForConfirmation=${nivAskedForConfirmation}, isExplicitGenerationCommand=${isExplicitGenerationCommand}, isExplicitConfirmation=${isExplicitConfirmation}`)
