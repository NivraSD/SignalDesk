...
async function generateWithGemini25Flash(request: ImageGenerationRequest) {
  ...
  try {
    ...
    if (data.candidates?.[0]?.content?.parts) {
      for (const part of data.candidates[0].content.parts) {
        if (part.inlineData?.data) {
          ...
          console.log('✅ Gemini 2.5 Flash Image (nano banana) generated successfully')
    ...
    console.error('Gemini 2.5 Flash Image error:', error)
    return null // Signal to try fallback
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'image') {
      // Try Gemini 2.5 Flash Image (nano banana) first, fall back to Imagen 3.0
      console.log('🚀 Trying Gemini 2.5 Flash Image for image generation...')
      result = await generateWithGemini25Flash(request)

      if (!result) {
        console.log('⚠️ Gemini 2.5 Flash Image failed, falling back to Imagen 3.0...')
