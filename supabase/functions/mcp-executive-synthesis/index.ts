...
// Vertex AI configuration (same as mcp-opportunity-detector-v2)
...
3. Recommended actions`;
  ...
  // Helper function to make Gemini API call with retry (using Google AI Studio, NOT Vertex AI)
  const callGeminiWithRetry = async (systemPrompt: string, userPrompt: string, maxRetries = 2): Promise<Response> => {
    // Use Google AI Studio API (free Flash) instead of Vertex AI ($$$)
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      // Retry on 429, 500, 503 (server errors)
      if ((status === 429 || status === 500 || status === 503) && attempt < maxRetries) {
        const delayMs = (attempt + 1) * 5000 + Math.random() * 3000;
        console.log(`⚠️ Gemini Flash ${status} error (attempt ${attempt + 1}/${maxRetries + 1}) - retrying in ${Math.round(delayMs/1000)}s...`);
  ...
  try {
    console.log('🚀 Calling Gemini 2.5 Flash via Vertex AI for executive synthesis...');
...
Remember: You are not writing a business report - you are writing a COMMUNICATIONS ACTION BRIEF.`;

    const response = await callGeminiWithRetry(systemPrompt, prompt);
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error('❌ Vertex AI Gemini Error:', status, errorData);
      } catch (e) {
        console.error('Failed to parse error response:', e);
      }
      throw new Error(`Vertex AI error: ${status}`);
    }

    const data = await response.json();
    const synthesisText = data.candidates?.[0]?.content?.parts?.[0]?.text || 'No synthesis generated';

    console.log('✅ Gemini 2.5 Flash response received, length:', synthesisText.length);
