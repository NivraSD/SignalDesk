...
/**
 * Intelligence Orchestrator V2 - Unified Executive Synthesis & Opportunity Engine
 * 
 * Flow:
 * 1. Receive monitoring data (raw articles)
 * 2. Send to monitoring-stage-2-enrichment for event/entity extraction
 * 3. Send enriched data to mcp-executive-synthesis for unified report
...
serve(async (req) => {
  ...
  try {
    ...
    const processingPromise = (async () => {
      try {
        console.log('🚀 Intelligence Orchestrator V2 - Request received');
    ...
    if (!organization && !organization_name) {
      return new Response(JSON.stringify({
        ...
        service: 'Intelligence Orchestrator V2',
    ...
    try {
      ...
      // CONSOLIDATED SYNTHESIS - Single call with comprehensive analysis
      ...
      const synthesisResponse = await callStage(
        ...
        {
          method: 'tools/call',  // MCP protocol format
          params: {
            ...
            arguments: {
              ...
              synthesis_focus: 'all_consolidated' // Get all perspectives in one call
            }
          }
        }
      );
