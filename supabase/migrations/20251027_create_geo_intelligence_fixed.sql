...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_geo_intelligence_org ON geo_intelligence(organization_id);
CREATE INDEX IF NOT EXISTS idx_geo_intelligence_signal_type ON geo_intelligence(signal_type);
CREATE INDEX IF NOT EXISTS idx_geo_intelligence_platform ON geo_intelligence(platform);
CREATE INDEX IF NOT EXISTS idx_geo_intelligence_priority ON geo_intelligence(priority);
CREATE INDEX IF NOT EXISTS idx_geo_intelligence_status ON geo_intelligence(status);
CREATE INDEX IF NOT EXISTS idx_geo_intelligence_created_at ON geo_intelligence(created_at DESC);

-- RLS Policies (service role will bypass these)
ALTER TABLE geo_intelligence ENABLE ROW LEVEL SECURITY;

-- Allow service role and authenticated users full access
CREATE POLICY "Allow service role full access" ON geo_intelligence
  FOR ALL
  USING (true)
  WITH CHECK (true);
