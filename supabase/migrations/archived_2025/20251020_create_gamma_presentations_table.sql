...
-- RLS Policies
CREATE POLICY "Users can view their organization's presentations"
  ON campaign_presentations FOR SELECT
  USING (organization_id IN (
    SELECT organization_id FROM profiles WHERE user_id = auth.uid()
  ));

CREATE POLICY "Users can insert presentations for their organization"
  ON campaign_presentations FOR INSERT
  WITH CHECK (organization_id IN (
    SELECT organization_id FROM profiles WHERE user_id = auth.uid()
  ));

CREATE POLICY "Users can update their organization's presentations"
  ON campaign_presentations FOR UPDATE
  USING (organization_id IN (
    SELECT organization_id FROM profiles WHERE user_id = auth.uid()
  ));

CREATE POLICY "Users can delete their organization's presentations"
