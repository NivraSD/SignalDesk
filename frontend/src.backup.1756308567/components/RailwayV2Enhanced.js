...
const RailwayV2Enhanced = () => {
  ...
  useEffect(() => {
    // Load organization from edge function (SINGLE SOURCE OF TRUTH)
    const loadOrganization = async () => {
      const orgData = await getUnifiedOrganization();
      
      // If no organization data exists, redirect to onboarding
      if (!orgData || !orgData.name) {
        console.log('📋 No organization data found in edge function, redirecting to onboarding...');
        navigate('/onboarding');
        return;
      }
      ...
      console.log('🏢 RailwayV2Enhanced loaded organization from edge function:', orgData);
      ...
      case 'intelligence':
        return <MultiStageIntelligence 
          ...
          onComplete={(intelligence) => {
            console.log('🎯 Intelligence complete:', intelligence);
            if (intelligence) {
